...
public class ProposalServiceImpl implements ProposalService {
    ...
    @Override
    public JsonResult findProposalPage(int reid, Integer pageNo, Integer pageSize,String status,Integer adid) {
        //设置页码和每页数量
        Page<Proposal> page = Page.of(pageNo, pageSize);
//把adminPage传入mapper查询中，adminPage就是分页结果
        Page<Proposal> proposalPage =  proposalMapper.selectProposalByRequestIdAndStatus(page,reid,status,adid);
...
//根据id查询Admin信息
        if(proposal.getId()==null||proposal.getId()<=0)
        {
            ...
            Request existRequest = requestMapper.selectById(proposal.getRequestId());
            if(existRequest==null)
            {return  JsonResult.fail("没有对应的需求号");}
            //新增
