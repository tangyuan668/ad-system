...
import {login} from '@/api/adProducer';
import { reactive,ref } from 'vue';
//导入userAdProducerStore函数
import { useAdProducerStore} from '@/stores/adProducer';
//导入userRouter函数
import { useRouter } from 'vue-router';
import { register } from '@/api/adProducer';
//获取adProducerStore对象
  ...
  loginFormRef.value.validate((valid, fields) => {
    if (valid) {
      ...
      login(loginForm).then((res) => {
        ...
        //后端返回的token数据赋值全局变量中
        adProducerStore.saveToken(res.message)
        //保存adProducer信息
        adProducerStore.saveAdProducer(res.data)
        //跳转/home页面
        router.push({
