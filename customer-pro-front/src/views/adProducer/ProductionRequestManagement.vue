...
import {productionRequestpage,deleteproductionRequest} from '@/api/productionRequest';
...
const handleQueryPage=()=>{
  productionRequestpage(queryPageParams).then((res)=>{
...
deleteproductionRequest(id).then(()=>{
...
if(!isSelected.value)
{  queryPageParams.pdid=adProducerStore.adProducer.id 
  console.log(`output->quer`,queryPageParams.pdid)
  handleQueryPage()
 }
else{
  queryPageParams.pdid=0
  handleQueryPage()
...
<template>
    ...
    <label for="'reid'">提案号</label>
  <el-input id="reid" v-model="queryPageParams.prid" style="width:240px"></el-input>
  <label for="'name'">名字</label>
  <el-input id="name" v-model="queryPageParams.name" style="width:240px"></el-input>
<!--   <label for="'name'">制作者号</label>
  <el-input id="name" v-model="queryPageParams.pdid" style="width:240px"></el-input> -->
  <el-button 
    round 
...
<!--   <el-button type="primary" round @click="handleAdd">新增</el-button> -->
  <el-button type="primary" round @click="back">返回</el-button>
    <el-table :data="tableData" style="width: 100%" stripe highlight-current-row>
      <el-table-column prop="proposalId" label="提案号" width="180" />

      <el-table-column prop="adproducerId" label="制作者号" width="180" />
      <el-table-column prop="name" label="需求名" width="180" />
  ...
  <template #default="scope">
    <span>
      {{ scope.row.content ? (scope.row.content.length > 20 ? scope.row.content.slice(0, 20) + '...' : scope.row.content) : '' }}
        ...
        <el-table-column prop="deadTime" label="截至时间" width="180" />
