...
import {signpage,deletesign} from '@/api/sign';
...
const handleQueryPage=()=>{
  signpage(queryPageParams).then((res)=>{
...
<template>
  ...
  <label for="'prid'">提案号</label>
  <el-input id="prid" v-model="queryPageParams.prid" style="width:240px"></el-input>
  ...
  <el-input id="name" v-model="queryPageParams.name" style="width:240px"></el-input>
    ...
    <el-table :data="tableData" style="width: 100%" stripe highlight-current-row>
      <el-table-column fixed prop="proposalId" label="提案号" width="180" />
...
<el-table-column label="客户签名" width="300">
        <!-- el-table-column插槽:scope.row获取到一行数据 -->
    <template #default="scope">
        
        <el-image style="width: 100px; height: 100px" :src="scope.row.customerSignature"  />
  ...
  <el-table-column label="广告商签名" width="300">
        <!-- el-table-column插槽:scope.row获取到一行数据 -->
    <template #default="scope">
        
        <el-image style="width: 100px; height: 100px" :src="scope.row.advertiserSignature"  />

    </template>
</el-table-column>
  <el-table-column prop="date" label="签署时间" width="180" />
