...
import {proposalpage,deleteproposal} from '@/api/proposal';
...
const handleQueryPage=()=>{
  proposalpage(queryPageParams).then((res)=>{
...
if(!isSelected.value)
{  queryPageParams.adid=store.advertiser.id 
  console.log(`output->quer`,queryPageParams.adid)
  handleQueryPage()
 }
else{
  queryPageParams.adid=0
  console.log(`output->quer`,queryPageParams.adid)
  handleQueryPage()
...
<el-select v-model="queryPageParams.status" placeholder="选择状态" style="width:240px" @change="changeStatus">
  ...
  <el-button type="primary" round @click="back">返回</el-button>
    <el-table :data="tableData" style="width: 100%" stripe highlight-current-row>
      <el-table-column prop="requestId" label="需求号" width="180" />
      <el-table-column prop="name" label="提案名" width="180" />
...
<!-- <template>
    <div>
      ...
      <el-table :data="proposalData" style="width: 100%" stripe highlight-current-row>
        ...
        <el-table-column label="操作" width="180">
          <template #default="scope">
            <el-button link type="primary" size="small" @click="handleEdit(scope.row.proposal_id)">
              编辑
            </el-button>
            <el-button link type="danger" size="small" @click="handleDelete(scope.row.proposal_id)">
              删除
            </el-button>
          </template>
        </el-table-column>
  ...
  import { fetchProposals, deleteProposal } from '@/api/proposal';
  import { useRouter } from 'vue-router';
  ...
  // 新增广告提案
  const handleAdd = () => {
    router.push({ path: '/advertiser/proposal/add' });
  };
  
  // 编辑广告提案
  const handleEdit = (id) => {
    router.push({ path: '/advertiser/proposal/edit', query: { id } });
  };
  
  // 删除广告提案
  const handleDelete = (id) => {
    deleteProposal(id).then(() => {
      loadProposals(); // 重新加载广告提案列表
    });
  };
  </script>
